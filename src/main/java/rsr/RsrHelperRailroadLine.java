 ...
 * @author morvael
 */
public final class RsrHelperRailroadLine extends RsrMapShaderBase {

	private HexControlInfo info;
	private Set<HexRef> supplySources;
	private HexRef destination;
	
	public RsrHelperRailroadLine(HexControlInfo info, Set<HexRef> supplySources) {
		super();
		this.info = info;
	...
	public void start(HashMap<HexRef, HashMap<String, TerrainMapShaderCost>> selectedHexes) {
		for (HexRef hr : supplySources) {
			if ((isRailroadSpace(hr)) && (info.canEnter(hr))) {
	...
	public boolean getCrossCost(HexRef fromHex, HexRef toHex, TerrainMapShaderCost currentCost) {
		...
		if (Filter.isAny(terrainMap.getLineTerrainName(fromHex, toHex), LINE_RAILROAD, LINE_CHONGARCAUSEWAY) == false) {
			return result(fromHex, false, 0.0d, false, null, 0);
		}
